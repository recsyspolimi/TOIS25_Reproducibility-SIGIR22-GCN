...
class INMODataReader(object):
    URM_DICT = {}
    ICM_DICT = {}
    UCM_DICT = {}

    def __init__(self, dataset_name, pre_splitted_path, fold, freeze_split = True):
        super(INMODataReader, self).__init__()

        pre_splitted_path += "data_split/"
        pre_splitted_filename = "splitted_data"
        
        dataset_dir = os.path.join("SIGIR2022/INMO_github/run/data/", dataset_name, str(fold))
        ...
        try:
            ...
            URM_train = self._load_data_file(os.path.join(dataset_dir, 'train.txt'))
            URM_test = self._load_data_file(os.path.join(dataset_dir, 'test.txt'))
            
            # The split is 70/10/20
