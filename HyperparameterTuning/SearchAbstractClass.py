...
class SearchAbstractClass(object):
    ...
    def _remove_intermediate_cases(self, cases_to_remove_list):
        ...
        if 0 in cases_to_remove_list:
            # If 0 has to be removed, reset everything
            self._init_metadata_dict(n_cases = len(self.metadata_dict['hyperparameters_df']),
                                     hyperparameter_names = self.metadata_dict['hyperparameters_df'].columns.tolist())

            if os.path.isfile(self.output_folder_path + self.output_file_name_root + "_best_model.zip"):
                os.remove(self.output_folder_path + self.output_file_name_root + "_best_model.zip")
                os.remove(self.output_folder_path + self.output_file_name_root + "SearchBayesianSkopt.txt")
            ...
            for index in cases_to_remove_list:
                ...
                self.metadata_dict['result_on_earlystopping_df'].drop([index], axis=0, inplace=True)
                self.metadata_dict['time_df'].loc[index] = np.nan

            if self.metadata_dict['hyperparameters_best_index'] >= min(cases_to_remove_list):
                # This case is problematic.
                # If the best validation case is to be removed, I do not have the saved model anymore. In this case reset everything
                print("Updating best case index")

                if self.save_model in ["no"]:
                    validation_result = self.metadata_dict['result_on_validation_df'].xs(self.cutoff_to_optimize, level=1, axis=0)
                    best_index = pd.to_numeric(validation_result[self.metric_to_optimize]).idxmax()

                    self.metadata_dict["hyperparameters_best"] = self.metadata_dict['hyperparameters_df'].loc[best_index].copy()
                    self.metadata_dict["hyperparameters_best_index"] = best_index
                    self.metadata_dict["result_on_validation_best"] = validation_result.loc[best_index].to_dict()

                    if self.evaluate_on_test in ["all", "best"]:
                        self.metadata_dict["result_on_test_best"] = self.metadata_dict['result_on_test_df'].loc[best_index].to_dict()

                elif self.save_model in ["best"]:
                    self._init_metadata_dict(n_cases = len(self.metadata_dict['hyperparameters_df']),
                                             hyperparameter_names = self.metadata_dict['hyperparameters_df'].columns.tolist())
        ...
        if os.path.isfile(self.output_folder_path + self.output_file_name_root + "_best_model_last.zip"):
            os.remove(self.output_folder_path + self.output_file_name_root + "_best_model_last.zip")
