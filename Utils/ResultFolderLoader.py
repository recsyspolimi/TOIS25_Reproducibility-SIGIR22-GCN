...
def _get_algorithm_feature_combinations(algorithm, algorithm_row_label, algorithm_file_name,
    ...
    for feature_matrix_name in feature_matrix_names_to_report_list:
        ...
        algorithm_data_to_print_list.append({
            "algorithm": algorithm,
            "algorithm_row_label": algorithm_row_label + " " + feature_matrix_row_label,
            "algorithm_file_name": algorithm_file_name + feature_matrix_name,
        })
...
def _convert_sec_list_into_biggest_unit(data_list):
    ...
    new_time_value, new_time_unit = seconds_to_biggest_unit(mean_sec)

    # Mean and std of an array are scaled by the same factor
    scaling_factor = new_time_value/mean_sec
    new_time_std = stddev_sec*scaling_factor
        ...
        if highlight_best:
            ...
            def _format_result_row_values(dataframe_row_series, dataframe_threshold_value, n_decimals):
                ...
                for dataset_column in dataframe_row_series.index.tolist():
                    ...
                    if dataset_column == "Algorithm" or pd.isna(result_value) or pd.isna(dataframe_threshold_value[dataset_column]):
                        continue
