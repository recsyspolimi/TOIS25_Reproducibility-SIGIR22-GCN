...
from SIGIR2022.HAKG_our_interface.HAKGDataReader import HAKGDataReader, HAKG_Add_KB_to_existing_split_DataReader
...
def run_this_algorithm_experiment(dataset_name,
    ...
    if dataset_name == "last-fm":
    ...
    elif dataset_name == "yelp2018" and split_type == "ours":
        ...
        dataset = HAKG_Add_KB_to_existing_split_DataReader(dataset_name, LightGCN_dataset, data_folder_path)

    else:
        dataset = HAKGDataReader(dataset_name, data_folder_path)
    ...
    #
    # recommender_instance = HAKG_RecommenderWrapper(URM_train, knowledge_base_df, use_gpu=False, use_cython_sampler=True)
    ...
    #         l2_reg = 1e-7,
    #         learning_rate = 1e-4,
    #         angle_loss_weight = 1e-3,
    #         contrastive_loss_margin = 0.8,
    #         GNN_layers_K = 2,
    #         add_inverse_relation = True,
    #         node_dropout_rate = 0.5,
    #         mess_dropout_rate = 0.5,
    #         n_negative_samples_M = 200,
    ...
    if dataset_name == 'alibaba-ifashion':
        ...
        dataset_hyperparameters = {
            ...
            "GNN_layers_K": 2,
            "n_negative_samples_M": 400,
            "contrastive_loss_margin": 0.7,
        }


    common_hyperparameters = {
        "epochs": 1000,
        "angle_loss_weight": 5*1e-3,
        'batch_size': 4096,
        'embedding_size': 64,
        'sgd_mode': "Adam",
        "l2_reg": 0.0,
    ...
    if flag_article_default:
        ...
        for fold_index in range(1,2):
            ...
            try:
                ...
                print("On recommender {} Exception {}".format(HAKG_RecommenderWrapper, str(e)))
                traceback.print_exc()
    ...
    if flag_print_results:
        ...
        if dataset_name == 'alibaba-ifashion':
            paper_results.loc[cutoff_to_optimize, 'RECALL'] = 0.1319
            paper_results.loc[cutoff_to_optimize, 'NDCG'] = 0.0848

        elif dataset_name == 'yelp2018':
            paper_results.loc[cutoff_to_optimize, 'RECALL'] = 0.0778
            paper_results.loc[cutoff_to_optimize, 'NDCG'] = 0.0501

        elif dataset_name == 'last-fm':
            paper_results.loc[cutoff_to_optimize, 'RECALL'] = 0.1008
            paper_results.loc[cutoff_to_optimize, 'NDCG'] = 0.0931

        else:
            paper_results = None


        reproduced_algorithm_list, base_algorithm_list = copy_reproduced_metadata_in_baseline_folder(HAKG_RecommenderWrapper,
