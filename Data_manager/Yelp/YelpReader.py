...
import ast, tarfile, os, shutil, zipfile
import pandas as pd
...
class YelpReader(DataReader):
    ...
    def _load_from_original_file(self):
        ...
        try:
            ...
            compressed_file = tarfile.open(compressed_file_folder + self.FILE_NAME, "r")
...
class Yelp2022Reader(YelpReader):

    DATASET_SUBFOLDER = "Yelp2022/"
    # https://www.kaggle.com/datasets/yelp-dataset/yelp-dataset

    IS_IMPLICIT = False
    FILE_NAME = "yelp_dataset_2022.zip"
    ...
    def _load_from_original_file(self):
        ...
        try:
            ...
            dataFile = zipfile.ZipFile(compressed_file_folder + self.FILE_NAME)
            dataFile.extract("yelp_academic_dataset_review.json", path=decompressed_file_folder + "decompressed/")
        ...
        self._print("Loading Interactions")
        URM_all_dataframe = self._loadURM(URM_path)
        # URM_all_dataframe = pd.read_json(URM_path, orient="records", lines=True)
        # URM_all_dataframe = URM_all_dataframe.drop_duplicates(subset=['UserID', 'ItemID'], keep='last')

        self._print("Removing duplicated interactions")
        URM_all_dataframe = URM_all_dataframe.groupby(['UserID', 'ItemID'], as_index=False )['Data'].max()
