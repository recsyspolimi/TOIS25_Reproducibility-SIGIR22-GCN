...
def run_this_algorithm_experiment(dataset_name,
    ...
    if dataset_name == "movielens100k":
        dataset = GDEDatasetReader("ml_100k", data_folder_path)
    elif dataset_name == "movielens1m":
        dataset = GDEDatasetReader("ml_1m", data_folder_path)
    else:
        dataset = GDEDatasetReader(dataset_name, data_folder_path)
    ...
    if dataset_name == 'citeulike':
        dataset_hyperparameters = {
            ...
            'beta': 5.0, # The paper reports 4.5, the GitHub repository 5.0
            'loss_type': 'adaptive',
            'epochs': 200,
        }
        ...
        dataset_hyperparameters = {
            ...
            'rough_ratio': 0.002,
            'feature_type': 'both',
            'beta': 4.0, # The paper reports 4.5, the GitHub repository 4.0
            'loss_type': 'bpr',
            'epochs': 50,
        }
        ...
        dataset_hyperparameters = {
            'learning_rate': 0.85, # The GitHub repository reports 0.85 / 0.12
            'drop_out': 0.2,
            'smooth_ratio': 0.2,
            'rough_ratio': 0.0,
            'feature_type': 'smoothed',
            'beta': 4.0,           # The GitHub repository reports 4.0 / 5.0
            'loss_type': 'adaptive',
            'epochs': 250,
        }
    ...
    if flag_article_default:
        ...
        for fold_index in range(1,6):
            ...
            try:
                ...
                print("On recommender {} Exception {}".format(GDE_RecommenderWrapper, str(e)))
                traceback.print_exc()
    ...
    if flag_print_results:
        paper_results = pd.DataFrame(index=[cutoff_to_optimize], columns=['PRECISION_RECALL_MIN_DEN', 'NDCG'])

        if dataset_name == 'citeulike':
            paper_results.loc[cutoff_to_optimize, 'NDCG'] = 0.1339
            paper_results.loc[cutoff_to_optimize, 'PRECISION_RECALL_MIN_DEN'] = 0.1224

        elif dataset_name == 'movielens1m':
            paper_results.loc[cutoff_to_optimize, 'NDCG'] = 0.5715
            paper_results.loc[cutoff_to_optimize, 'PRECISION_RECALL_MIN_DEN'] = 0.5423

        elif dataset_name == 'movielens100k':
            paper_results.loc[cutoff_to_optimize, 'NDCG'] = 0.5731
            paper_results.loc[cutoff_to_optimize, 'PRECISION_RECALL_MIN_DEN'] = 0.5400

        elif dataset_name == 'pinterest':
            paper_results.loc[cutoff_to_optimize, 'NDCG'] = 0.1240
            paper_results.loc[cutoff_to_optimize, 'PRECISION_RECALL_MIN_DEN'] = 0.1147

        elif dataset_name == 'gowalla':
            paper_results.loc[cutoff_to_optimize, 'NDCG'] = 0.1632
            paper_results.loc[cutoff_to_optimize, 'PRECISION_RECALL_MIN_DEN'] = 0.1449
        else:
            paper_results = None


        reproduced_algorithm_list, base_algorithm_list = copy_reproduced_metadata_in_baseline_folder(GDE_RecommenderWrapper,
        ...
        result_loader.generate_latex_results(result_folder_path + "{}_{}_{}_latex_results.txt".format(ALGORITHM_NAME, dataset_name, "article_metrics"),
                                             metrics_list=['PRECISION_RECALL_MIN_DEN', 'NDCG'],
...
if __name__ == '__main__':
    ...
    dataset_list = ["movielens100k", "movielens1m", "citeulike", "pinterest", "gowalla"]
