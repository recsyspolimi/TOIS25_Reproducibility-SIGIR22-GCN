...
def run_this_algorithm_experiment(dataset_name,
    ...
    if dataset_name == "amazon-book":
        dataset = HCCFDataReader("amazon", data_folder_path)
    elif dataset_name == "yelp2018":
        dataset = HCCFDataReader("yelp", data_folder_path)
    elif dataset_name == "movielens10m":
        dataset = HCCFDataReader("ml10m", data_folder_path)
    else:
        raise ValueError()
    ...
    # recommender_instance = HCCF_RecommenderWrapper(URM_train, use_gpu = False, use_cython_sampler = True)
    ...
    #         "HYP_layers_C": 3,
    #         "dropout": 0.5,
    #         "contrastive_loss_temperature_tau": 1,
    #         "contrastive_loss_weight": 1e-3,
    # })
    #
    ...
    if dataset_name == "amazon-book":
        dataset_hyperparameters = {
            "dropout": 0.5,
            "contrastive_loss_weight": 1e-7,
            "l2_reg": 1e-2,
            "contrastive_loss_temperature_tau": 0.1,
            "batch_size": 2048, # The original value is 256 but this results in prohibitive computational cost (circa 3 days for amazon books)
        }

    elif dataset_name == "yelp2018":
        dataset_hyperparameters = {
            "dropout": 0.5,
            "contrastive_loss_weight": 1e-4,
            "l2_reg": 1e-3,
            "contrastive_loss_temperature_tau": 1.0,
            "batch_size": 256,
        }
        ...
        dataset_hyperparameters = {
            ...
            "l2_reg": 1e-5,
            "contrastive_loss_temperature_tau": 0.1,
            "batch_size": 2048, # The original value is 256 but this results in prohibitive computational cost (circa 3 days for movielens10m)
        }
    ...
    max_epochs_for_earlystopping = 300
    ...
    if flag_article_default:
        ...
        for fold_index in range(1,2):
            ...
            try:
                ...
                print("On recommender {} Exception {}".format(HCCF_RecommenderWrapper, str(e)))
                traceback.print_exc()
    ...
    if flag_print_results:
        paper_results = pd.DataFrame(index=[20, 40], columns=['RECALL', 'NDCG'])

        if dataset_name == 'yelp2018':
            paper_results.loc[20, 'RECALL'] = 0.0607
            paper_results.loc[20, 'NDCG'] = 0.0510

            paper_results.loc[40, 'RECALL'] = 0.1007
            paper_results.loc[40, 'NDCG'] = 0.0658

        elif dataset_name == 'amazon-book':
            paper_results.loc[20, 'RECALL'] = 0.0344
            paper_results.loc[20, 'NDCG'] = 0.0258

            paper_results.loc[40, 'RECALL'] = 0.0561
            paper_results.loc[40, 'NDCG'] = 0.0330

        elif dataset_name == 'movielens10m':
            paper_results.loc[20, 'RECALL'] = 0.2048
            paper_results.loc[20, 'NDCG'] = 0.2467

            paper_results.loc[40, 'RECALL'] = 0.3081
            paper_results.loc[40, 'NDCG'] = 0.2717
        else:
            paper_results = None


        reproduced_algorithm_list, base_algorithm_list = copy_reproduced_metadata_in_baseline_folder(HCCF_RecommenderWrapper,
...
if __name__ == '__main__':
    ...
    dataset_list = ["yelp2018", "amazon-book", "movielens10m"]
